...
- 复用 IndexedDB 库 `yuki-books`，版本号 +1 新增 objectStore `token-cache`，记录形如 `{ id, bookId, rows }`；`id = bookId + ':' + chapterIndex + ':' + hash(本章全文)`；为 `bookId` 建索引，为将来可能的删书入口预留清理路径（当前项目无删书入口，本期不实现清理）
...
- 后端零改动，38 用例保持不动
- 浏览器端到端：上传轻小说文本走完「判定 → 顶栏显示 → 译文风格 → 手动改标签 → 不再被覆盖」；《こころ》不触发判定；重开同一本书分词秒开；隐私模式降级正常
...
- 每书术语 / 人名表注入翻译 prompt
- 删书入口（届时按 `bookId` 索引清理 `token-cache`）
